...
void core_init_mmu_map(unsigned long seed, struct core_mmu_config *cfg, long offset);
...
#else
static inline paddr_t core_mmu_get_tee_ram_start(void)
{
	return TEE_RAM_START;
}

static inline paddr_t core_mmu_get_ta_ram_start(void)
{
	return TA_RAM_START;
}
#endif
