...
static long phy_tee_ram_offset = 0;
static bool core_mmu_is_enabled = false;
#endif
				      ...
				      size_t nelems)
{
	...
	paddr_t tee_ram_start = core_mmu_get_tee_ram_start();
	paddr_t ta_ram_start = core_mmu_get_ta_ram_start();
...
static void fixup_sec_mem_config(long offset)
{
	size_t n = 0;

	for (n = 0; n < ARRAY_SIZE(secure_only); n++)
		secure_only[n].paddr += offset;
}
				 ...
				 size_t num_elems, long offset)
{
	...
	for (mem = phys_mem_map_begin; mem < phys_mem_map_end; mem++) {
		...
		if ((m.addr >= TEE_RAM_START && m.addr <
					(TEE_RAM_START + TEE_RAM_PH_SIZE)) ||
			(m.addr >= TA_RAM_START && m.addr <
...
static unsigned long init_mem_map(struct tee_mmap_region *memory_map,
				  size_t num_elems, unsigned long seed, long offset)
{
	/*
	 ...
	 * addresses.
	 */
	vaddr_t id_map_start = (vaddr_t)(__identity_map_init_start + offset);
	vaddr_t id_map_end = (vaddr_t)(__identity_map_init_end + offset);
	unsigned long offs = 0;
	size_t last = 0;

	last = collect_mem_ranges(memory_map, num_elems, offset);
	...
	if (IS_ENABLED(CFG_CORE_ASLR) && seed) {
		vaddr_t base_addr = TEE_RAM_START + offset + seed;
	...
	if (!assign_mem_va(TEE_RAM_START + offset, memory_map))
		panic();
 ...
 * the unpaged area.
 */
void __weak core_init_mmu_map(unsigned long seed, struct core_mmu_config *cfg, long offset)
{
#ifndef CFG_VIRTUALIZATION
	...
	offs = init_mem_map(tmp_mmap, ARRAY_SIZE(static_memory_map), seed, offset);

	check_mem_map(tmp_mmap);
	core_init_mmu(tmp_mmap);
	dump_xlat_table(0x0, CORE_MMU_BASE_TABLE_LEVEL);
	core_init_mmu_regs(cfg, offset);
	cfg->load_offset = offs;
	memcpy(static_memory_map, tmp_mmap, sizeof(static_memory_map));
#ifdef CFG_WITH_VIRTUAL_TEE_RAM_START
	core_mmu_is_enabled = true;
#endif
}

#ifdef CFG_WITH_VIRTUAL_TEE_RAM_START
paddr_t core_mmu_get_tee_ram_start(void)
{
	return TEE_RAM_START + phy_tee_ram_offset;
}

paddr_t core_mmu_get_ta_ram_start(void)
{
	return TA_RAM_START + phy_tee_ram_offset;
}
#endif
